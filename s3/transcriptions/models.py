...
class AudioChunk(BaseModel):
    ...
    # ASR-related fields
    asr_transcript = models.TextField(blank=True, null=True, help_text="Transcript generated by ASR model")
    asr_status = models.CharField(
        max_length=20, 
        choices=ASR_STATUS_CHOICES,
        default="pending",
        null=True, 
        blank=True,
        help_text="Current status of ASR processing"
    )
    wer = models.FloatField(
        null=True, 
        blank=True, 
        help_text="Word Error Rate between ASR transcript and final feature text"
    )
    ...
    def calculate_wer(self):
        """
        Calculate Word Error Rate between ASR transcript and final feature text.
        Updates the WER field if both transcripts are available.
        """
        if self.asr_transcript and self.feature_text:
            from jiwer import wer
            
            # Calculate WER using the jiwer library
            try:
                self.wer = wer(self.feature_text, self.asr_transcript)
                self.save(update_fields=['wer'])
                return self.wer
            except Exception as e:
                logger.error(f"Error calculating WER for chunk {self.unique_id}: {str(e)}")
                return None
        return None
