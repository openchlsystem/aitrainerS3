        ...
        if current_chunk_start is not None:
            if is_silent and (is_previous_frame_silent or i + frame_length_samples - current_chunk_start >= max_chunk_length_samples):
                if last_valid_end and last_valid_end - current_chunk_start >= min_chunk_length_samples:
                    chunks.append((current_chunk_start, last_valid_end + overlap_samples))
                    current_chunk_start = None
            else:
                last_valid_end = i + frame_length_samples
                if not is_silent and i + frame_length_samples - current_chunk_start >= max_chunk_length_samples:
                    chunks.append((current_chunk_start, min(len(audio), last_valid_end + overlap_samples)))
                    current_chunk_start = None
...
def split_and_save_chunks(cleaned_audio_obj, output_format='wav', min_chunk_length_ms=3000, max_chunk_length_ms=7000, frame_length_ms=30, sr=16000, overlap_ms=2000):
    input_file = cleaned_audio_obj.audio_file.path
    ...
    min_chunk_length_samples = samples_per_ms(sr, adjust_to_frame_length(min_chunk_length_ms, frame_length_ms))
    max_chunk_length_samples = samples_per_ms(sr, adjust_to_frame_length(max_chunk_length_ms, frame_length_ms))
    chunks = analyze_chunks(y, sr, min_chunk_length_samples, max_chunk_length_samples, frame_length_samples, overlap_samples)

    chunk_paths = []
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(save_chunk, y, sr, chunk, i, output_dir, file_prefix, output_format)
